...
const prefetchPemeriksaanAddPage = () =>
  import("../../pages/add_examination_page");
...
export function RecentPemeriksaan({
  ...
  const { data: kunjunganData, isLoading: isKunjunganLoading } =
    useKunjunganList({ limit: 15 });
  const { data: pesertaData, isLoading: isPesertaLoading } = usePesertaList({
    limit: 100,
  });
  ...
  const { paginatedItems, totalPages, totalItems } = useMemo(() => {
    ...
    const merged = [...kunjunganList]
      .sort(
        (a, b) =>
          new Date(b.tanggal_kunjungan).getTime() -
          new Date(a.tanggal_kunjungan).getTime(),
      )
      .map((item) => {
    ...
    const paginatedItems = merged.slice(
      startIndex,
      ...
      <div className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {paginatedItems.map((item) => {
            const kategori = (item.peserta_kategori ||
              "produktif") as KategoriKey;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span
                    className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${config.bg} ${config.color}`}
                  >
                    {config.label}
                  </span>
                </div>
