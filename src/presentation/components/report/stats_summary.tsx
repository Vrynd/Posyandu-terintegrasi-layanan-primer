...
import { MONTH_NAMES_FULL, STAT_ITEMS_CONFIG, type StatConfig } from './config';
...
interface StatItemProps extends StatConfig {
   value: string;
   subtext: string;
}
...
function StatsHeader({ monthName, year }: StatsHeaderProps) {
   return (
...
function StatItemCard({ icon: Icon, label, value, subtext, color, bgColor }: StatItemProps) {
   return (
      <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
         <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[13px] text-gray-500 font-normal tracking-wide">{label}</span>
         </div>
...
export function StatsSummary({ month, year }: StatsSummaryProps) {
   ...
   // Pengolahan dan penyaringan data berdasarkan bulan/tahun terpilih
   const stats = useMemo(() => {
      if (!pesertaResponse || !pemeriksaanResponse) return null;
      ...
      // Filter kunjungan berdasarkan bulan/tahun
      ...
      // Filter peserta yang aktif melakukan kunjungan di bulan ini
      ...
      // Hitung rincian lokasi pemeriksaan
      ...
      // Hitung distribusi peserta berdasarkan kategori
      ...
      // Cari kategori dengan jumlah peserta terbanyak
   ...
   const statItems = useMemo(() => {
      ...
      return STAT_ITEMS_CONFIG.map(config => {
         let value = '-';
         let subtext = 'belum ada data';
   ...
   return (
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden mb-4">
         <StatsHeader monthName={MONTH_NAMES_FULL[month - 1]} year={year} />
         <div className="p-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
