...
function TypeSelectionCard({ report, isSelected, onToggle }: TypeSelectionCardProps) {
   const IconComponent = report.icon;

   return (
      <button
         onClick={() => onToggle(report.id as ReportTypeId)}
         className={`relative p-5 rounded-xl cursor-pointer border text-left transition-all ${isSelected
      ...
      >
         ...
         <div className="flex flex-wrap gap-1.5">
            {report.details.map((detail, idx) => (
               <span key={idx} className="text-xs px-2.5 py-1.5 bg-slate-800 text-white rounded-full">
                  {detail}
               </span>
            ))}
         </div>
...
export function ReportType({ reports, selectedReport, onToggle }: ReportTypeProps) {
   return (
