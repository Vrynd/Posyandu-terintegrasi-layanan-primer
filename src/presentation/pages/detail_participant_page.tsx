...
export function DetailParticipantPage() {
  ...
  const searchPath = location.state?.search
    ? `/dashboard/participants?${location.state.search}`
  ...
  const { data: lastVisit, isLoading: isLastVisitLoading } =
    useLatestVisit(numericId);
  ...
  // Dynamic page title
  useDocumentTitle(
    peserta?.nama ? `${peserta.nama}` : "Detail Peserta",
  );
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 leading-relaxed capitalize">
            detail informasi peserta dan riwayat pemeriksaan
          </p>
        </div>
      ...
      <PesertaLastVisitCard
        lastVisit={lastVisit}
        isLoading={isLastVisitLoading}
        onNavigate={handleNavigateToPemeriksaan}
      />
